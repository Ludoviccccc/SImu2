...
* Once we obtain a vector $[\mbox{ir}(m),\cdots,\mbox{ir}(m)]$ made with the reward of each module, we crate a probability distribution $p = [\frac{\mbox{ir}(m)}{\sum_{m}\mbox{ir}},\cdots,\frac{\mbox{ir}(m)}{\sum_{m}\mbox{ir}}]$ to selected the module to explore. We also merge it so that it selects a module with a discrete uniform random low when $\epsilon\sim \mathcal{B}(p)$, with e.g $p=0.1$ 
